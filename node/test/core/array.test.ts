...
describe('mlx.core.array', () => {
  ...
  describe('mlx.core.full', () => {
    ...
    it('creates array with integer value', () => {
      const x = mlx.core.full([2, 2], 5, 'int32');
    ...
    it('creates array with boolean dtype', () => {
      const x = mlx.core.full([2], 1, 'bool');
      assert.equal(x.dtype, 'bool');
      assert.deepEqual(x.toArray(), [true, true]);
    });

    it('infers int32 dtype from integer scalar', () => {
      const x = mlx.core.full([3], 42);
      assert.equal(x.dtype, 'int32');
      assert.deepEqual(x.toArray(), [42, 42, 42]);
    });

    it('infers float32 dtype from float scalar', () => {
      const x = mlx.core.full([2], 3.14);
      assert.equal(x.dtype, 'float32');
      assert.ok(
        x.toArray().every((val) => Math.abs((val as number) - 3.14) < 0.001),
      );
    });

    it('broadcasts array value across shape', () => {
    ...
    it('broadcasts TypedArray value', () => {
      const value = new Float32Array([2.0, 3.0]);
