...
Napi::Object WrapArray(
  ...
  auto external = Napi::External<std::shared_ptr<mlx::core::array>>::New(
      ...
      [](Napi::Env, std::shared_ptr<mlx::core::array>* data) { delete data; });
...
Napi::Value Arange(const Napi::CallbackInfo& info) {
  ...
  try {
    if (mlx::core::issubdtype(dtype, mlx::core::floating)) {
      ...
      int64_t start =
          two_positional ? to_i64(*start_v) : static_cast<int64_t>(0);
...
Napi::Value Subtract(const Napi::CallbackInfo& info) {
  auto env = info.Env();
  if (info.Length() < 2) {
    Napi::TypeError::New(env, "subtract expects two arguments")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
...
Napi::Value Where(const Napi::CallbackInfo& info) {
  ...
  if (env.IsExceptionPending() || condition == nullptr || x == nullptr ||
      y == nullptr) {
    return env.Null();
  }
  auto streamArg = GetStreamArgument(info, 3);
  ...
  auto tensor = std::make_shared<mlx::core::array>(mlx::core::where(
      condition->tensor(), x->tensor(), y->tensor(), streamArg));
  return WrapArray(env, tensor);
}
